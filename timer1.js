...
for (let duration of durations) {
  ...
  let seconds = Number(duration);  //Make each imput a number
 
  if (typeof seconds !== 'number' || isNaN(seconds) || seconds < 0) { //check if each imput is actually a valid number, if not log that its not a number
